"""
Endpoints principales de autenticación para el sistema Ezekl Budget.
Maneja el flujo de login de 2 pasos con tokens temporales y JWE, Microsoft OAuth callbacks.
"""
...
async def request_token(data: RequestTokenRequest):
    ...
    try:
        ...
        if user_email:
            ...
            # Agregar email a la cola para envío en background
...
async def microsoft_callback_redirect(
    ...
    try:
        ...
        if not result or not result.get("success"):
            error_msg = result.get("message", "Error desconocido") if result else "Respuesta vacía"
        ...
        # 7. Verificar estado de asociación
        ...
        if association_status == "associated":
            ...
            logger.warning(
                f"⚠️ Usuario Microsoft no asociado: {microsoft_user.get('email')}"
            )
            html_content = templates.TemplateResponse(
        ...
        if not user_login_data or not isinstance(user_login_data, dict):
            logger.error(f"❌ Datos inválidos del usuario")
            html_content = templates.TemplateResponse(
                "auth/microsoft_generic_error.html",
                {
                    "request": {},
        ...
        # Redirigir a la URL personalizada con el token
...
async def microsoft_callback_whatsapp(
    ...
    try:
        # 1. Manejo de errores
        if error:
            logger.error(f"Error de Microsoft OAuth (WhatsApp): {error} - {error_description}")
            return _create_whatsapp_error_page("Error de Autenticación", error_description or error)

        if not code:
            logger.error("No se recibió código de autorización (WhatsApp)")
            return _create_whatsapp_error_page("Error de Autenticación", "No se recibió código de autorización.")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(token_url, data=token_data) as token_response:
                if token_response.status != 200:
                    logger.error("Error obteniendo token (WhatsApp)")
                    return _create_whatsapp_error_page("Error de Autenticación", "Error al obtener token.")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(graph_url, headers=headers) as user_response:
                if user_response.status != 200:
                    logger.error("Error obteniendo datos de usuario (WhatsApp)")
                    return _create_whatsapp_error_page("Error de Autenticación", "Error al obtener datos del usuario.")
        ...
        if not result.get("success"):
            logger.error(f"Error en stored procedure (WhatsApp): {result.get('message')}")
            return _create_whatsapp_error_page("Error de Autenticación", "Error procesando autenticación.", 500)

        # 5. Procesar según estado de asociación
        association_status = result.get("associationStatus", "unknown")
        microsoft_user_info = result.get("microsoftUser", {})
        ...
        wa_number = (
            bot_phone_number.replace("+", "").replace(" ", "").replace("-", "")
            if bot_phone_number
        ...
        if association_status == "needs_association":
            ...
            logger.info(f"✅ Usuario WhatsApp autenticado: {phone_number}")
            ...
            logger.error(f"Estado de asociación desconocido: {association_status}")
            return _create_whatsapp_error_page("Error del Servidor", "Estado desconocido.", 500)

    except Exception as e:
        logger.error(f"Error en microsoft_callback_whatsapp: {str(e)}")
        return _create_whatsapp_error_page("Error del Servidor", "Error inesperado.", 500)
    ...
    if state:
        try:
            ...
            if state_data.get("flow_type") == "url_redirect" and state_data.get("flow_id"):
                ...
                logger.info(f"🔄 Flujo de URL redirect: {state_data.get('flow_id')}")

            elif state_data.get("whatsapp_token") and state_data.get("phone_number"):
                is_whatsapp_auth = True
                whatsapp_data = state_data
                logger.info(f"📱 Flujo WhatsApp: {state_data.get('phone_number')}")

        except Exception as e:
            logger.warning(f"No se pudo decodificar state: {str(e)}")
