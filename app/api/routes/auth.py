    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(token_url, data=token_data) as token_response:
                if token_response.status != 200:
                    ...
                    return _create_whatsapp_error_page("Error de Autenticación", "Error al obtener token de Microsoft.")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(graph_url, headers=headers) as user_response:
                if user_response.status != 200:
                    error_text = await user_response.text()
                    logger.error(f"Error obteniendo datos de usuario (WhatsApp): {error_text}")
        ...
        if not result.get("success"):
            ...
            return _create_whatsapp_error_page("Error de Autenticación", "Error en el procesamiento de la autenticación.", 500)
        ...
        return _create_whatsapp_error_page("Error del Servidor", "Ocurrió un error inesperado.", 500)
